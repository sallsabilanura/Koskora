<x-app-layout>
    ...
    <div class="max-w-4xl mx-auto space-y-6">
        <div class="flex items-center justify-between">
            <h2 class="text-2xl font-bold text-slate-800">Tambah Data Sewa Baru</h2>
            ...
            <form action="{{ route('rentals.store') }}" method="POST" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="date" name="start_date" id="start_date" class="w-full rounded-xl border-slate-200 focus:border-blue-500 focus:ring-blue-500 transition-colors" value="{{ old('start_date') }}" required onchange="calculateRental()">
                    </div>

                    <!-- Duration -->
                    <div>
                        <label class="block text-sm font-bold text-slate-700 mb-2">Durasi Sewa (Bulan)</label>
                        <input type="number" name="duration_months" id="duration_months" class="w-full rounded-xl border-slate-200 focus:border-blue-500 focus:ring-blue-500 transition-colors" value="{{ old('duration_months') }}" min="1" required oninput="calculateRental()">
                    </div>

                    <!-- End Date (Read-only) -->
                    <div>
                        <label class="block text-sm font-bold text-slate-700 mb-2">Tanggal Selesai (Otomatis)</label>
                        <input type="date" name="end_date" id="end_date" class="w-full rounded-xl border-slate-100 bg-slate-50 text-slate-500 cursor-not-allowed" value="{{ old('end_date') }}" readonly>
                    </div>

                    <!-- Total Price (Read-only) -->
                    <div>
                        <label class="block text-sm font-bold text-slate-700 mb-2">Total Harga (Tahunan/Bulanan)</label>
                        <input type="text" id="total_price_display" class="w-full rounded-xl border-slate-100 bg-slate-50 text-slate-800 font-bold cursor-not-allowed" readonly>
                    </div>
