<x-app-layout>
    @section('header_title', 'Edit Payment')

    <div class="max-w-4xl mx-auto space-y-6">
        <div class="flex items-center justify-between">
            <h2 class="text-2xl font-bold text-slate-800">Edit Data Pembayaran Sewa</h2>
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-8">
            <form action="{{ route('rent-payments.update', $rentPayment->id) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        <label class="block text-sm font-bold text-slate-700 mb-2">Pilih Sewa</label>
                        <select name="rental_id" class="w-full rounded-xl border-slate-200 focus:border-blue-500 focus:ring-blue-500 transition-colors">
                            @foreach ($rentals as $rental)
                                <option value="{{ $rental->id }}" {{ old('rental_id', $rentPayment->rental_id) == $rental->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <input type="text" name="month" value="{{ old('month', $rentPayment->month) }}" class="w-full rounded-xl border-slate-200 focus:border-blue-500 focus:ring-blue-500 transition-colors" placeholder="Contoh: Januari 2024">
                    </div>

                    <!-- Amount -->
                    <div>
                        <label class="block text-sm font-bold text-slate-700 mb-2">Jumlah Bayar (Rp)</label>
                        <input type="number" step="0.01" name="amount" value="{{ old('amount', $rentPayment->amount) }}" class="w-full rounded-xl border-slate-200 focus:border-blue-500 focus:ring-blue-500 transition-colors">
                    </div>

                    <!-- Date -->
                    <div>
                        <label class="block text-sm font-bold text-slate-700 mb-2">Tanggal Pembayaran</label>
                        <input type="date" name="payment_date" value="{{ old('payment_date', $rentPayment->payment_date) }}" class="w-full rounded-xl border-slate-200 focus:border-blue-500 focus:ring-blue-500 transition-colors">
                    </div>

                    <!-- Method -->
                    <div>
                        <label class="block text-sm font-bold text-slate-700 mb-2">Metode Pembayaran</label>
                        <input type="text" name="method" value="{{ old('method', $rentPayment->method) }}" class="w-full rounded-xl border-slate-200 focus:border-blue-500 focus:ring-blue-500 transition-colors" placeholder="Contoh: Transfer, Cash">
                    </div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <select name="status" class="w-full rounded-xl border-slate-200 focus:border-blue-500 focus:ring-blue-500 transition-colors">
                            <option value="paid" {{ old('status', $rentPayment->status) == 'paid' ? 'selected' : '' }}>Paid</option>
                            <option value="unpaid" {{ old('status', $rentPayment->status) == 'unpaid' ? 'selected' : '' }}>Unpaid</option>
                        </select>
                    </div>
                </div>
