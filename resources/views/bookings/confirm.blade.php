<x-app-layout>
    ...
    <div class="max-w-4xl mx-auto py-8 px-4 sm:px-6">
        <div class="text-center mb-8">
            ...
            <h2 class="text-2xl sm:text-3xl font-extrabold text-slate-800 tracking-tight mt-2">Konfirmasi Penyewaan</h2>
            <p class="text-slate-500 mt-2 text-sm">Silakan tinjau kembali data penyewaan Anda sebelum menekan tombol konfirmasi.</p>
        </div>

        <div class="bg-white rounded-3xl border border-slate-200 shadow-xl overflow-hidden">
            <div class="p-5 sm:p-8 md:p-10 space-y-8">
                <!-- Summary Grid -->
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-12">
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="bg-blue-50 rounded-2xl p-5 border border-blue-100 flex items-center justify-between">
                            <div>
                                <div class="text-lg font-black text-blue-900">Kamar {{ $room->room_number }}</div>
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="bg-slate-50 rounded-2xl p-5 border border-slate-200">
                            <div class="text-lg font-bold text-slate-800">{{ \Carbon\Carbon::parse($startDate)->translatedFormat('d F Y') }}</div>
                            <div class="text-sm text-slate-500">Estimasi sampai <span id="display-end-date">{{ \Carbon\Carbon::parse($startDate)->addMonth()->translatedFormat('d F Y') }}</span></div>
                        </div>
                    </div>
                </div>

                <!-- Profile Review -->
                <div class="px-5 sm:px-8 py-6 rounded-2xl bg-slate-50/50 border border-slate-100 space-y-5">
                    <div class="flex items-center justify-between">
                        <div class="text-xs font-bold text-slate-400 uppercase tracking-wider">Profil Penyewa</div>
                        <a href="{{ route('profile.edit') }}" class="text-[10px] font-black text-blue-600 uppercase hover:underline">Ubah Data</a>
                    </div>

                    <!-- Photo Thumbnails -->
                    <div class="flex gap-3">
                        @if($user->tenant->foto_diri)
                            <div class="w-16 h-16 sm:w-20 sm:h-20 rounded-2xl overflow-hidden border-4 border-white shadow-sm flex-shrink-0">
                    ...
                    <!-- Data Grid — 1 col on mobile, 2 col on sm+, 3 col on md+ -->
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                        ...
                        <div>
                            ...
                            <div class="text-sm font-black text-slate-800 font-mono tracking-tighter break-all">{{ $user->tenant->nik }}</div>
                        </div>
                        <div>
                ...
                <div class="pt-6 border-t border-slate-100">
                    <form action="{{ route('bookings.store', $room->id) }}" method="POST" class="space-y-6">
                        @csrf
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <!-- Bulanan Card -->
                                <label class="relative flex flex-col p-5 bg-white border-2 border-blue-600 rounded-2xl cursor-pointer hover:shadow-md transition-all duration-200" id="label-monthly">
                                    ...
                                    <div class="flex items-center justify-between">
                                        ...
                                        <div class="w-4 h-4 rounded-full border-2 border-blue-600 flex items-center justify-center bg-blue-600 flex-shrink-0" id="radio-dot-monthly">
                                ...
                                <!-- Tahunan Card -->
                                <label class="relative flex flex-col p-5 bg-white border-2 border-slate-200 rounded-2xl cursor-pointer hover:shadow-md transition-all duration-200" id="label-yearly">
                                    ...
                                    <div class="flex items-center justify-between">
                                        ...
                                        <div class="w-4 h-4 rounded-full border-2 border-slate-300 flex items-center justify-center flex-shrink-0" id="radio-dot-yearly">
                        ...
                        <div class="bg-blue-50 rounded-2xl p-5 border border-blue-100 flex items-center justify-between">
                            ...
                            <div class="text-right">
                                ...
                                <div class="text-sm font-extrabold text-blue-900 mt-1" id="end-date-text">{{ \Carbon\Carbon::parse($startDate)->addMonth()->translatedFormat('d F Y') }}</div>
                            </div>
                        </div>

                        <button type="submit" class="w-full py-4 sm:py-5 bg-emerald-600 text-white rounded-2xl font-black text-base sm:text-lg hover:bg-emerald-700 shadow-2xl shadow-emerald-100 transition-all flex items-center justify-center space-x-3">
    ...
    <script>
        function updateDurationSelection(type) {
            ...
            if (type === 'monthly') {
                ...
                document.getElementById('display-end-date').innerText = formatDate(d);
            } else {
